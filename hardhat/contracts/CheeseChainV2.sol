...
contract CheeseChainV2 {
    ...
    function addStep(uint lotNumber, Coordinates calldata coordinates) onlyBasicParticipant lotExists(lotNumber) external {
        totalSteps += 1;
        steps[totalSteps] = Step(msg.sender, lots[lotNumber].lastStep, block.timestamp, coordinates);
